...
*/
int hash(int x,int cont,int max){
     int aleatorio;
     aleatorio= ((x*cont+13)*23209)%max;
     if(aleatorio <= 1 || aleatorio == max){
         aleatorio=hash(x+1,cont+7,max);
         return aleatorio;
    }
    else{
     return aleatorio;
...
void main() {
     ...
     colorDraw = 0;
     while(1){
          drawPixel(colorDraw,hash(suma+1,suma,MAX_COLS),hash(suma-1,suma,MAX_ROWS));
          suma++;
          i++;
          j++;
          colorDraw++;
